...
#Début du script réalisé avec Manu
...
Metabo3 <- Metabo3[-1,] #Supprimer le nom de la ligne maintenant que la matrice a été transposée (= changée de sens)
...
fix(Metabo4)
#Que JB
Metabo_species <- Metabo4[is.na(Metabo4$Altitude),] # exclude rubra récoltées sur un gradient altitudinal
colnames(Metabo4)
Metabo_species2 <- Metabo_species[,29:ncol(Metabo_species)] #Nom des molécules commence à partir de colonne 29 incluse jusqu'à la fin
...
reg = lm(div_species~Metabo_species$Elevation)
anova(reg)
...
Metabo_rubra <- Metabo4[!is.na(Metabo4$Altitude),] #exclus les plantes non récoltées sur un gradient altitudinal
Metabo_rubra2 <- Metabo_rubra[,29:ncol(Metabo_species)] #Nom des molécules commence à partir de colonne 29 incluse jusqu'à la fin
...
ggplot(data=Metabo_rubra, aes(x=Altitude.2, y = div_rubra))+
  geom_boxplot(aes(group= Altitude.2, fill = Altitude.2))+
  labs(title="Phytochemistry richness according to elevation",
       subtitle="P-value = 0.04349",
       x="Elevation",
       y="Phytochemistry richness")
...
Metabo_species <- Metabo4[!is.na(Metabo4$Altitude),] #exclus les plantes non récoltées sur un gradient altitudinal
...
Metabo_species2_env <- Metabo_species[,3:28] #on garde les colonnes 3 à 28 pour créer la matrice environnementale
Metabo_species2_env$div_sp <- div_species #Rajouter la colonne div_sp qui contient les éléments de div_species (voir au dessus)
...
#calculer une matrice de distance en méthode Bray curtis ----
#Bray curtis = structures chimiques proches, pas richesse !!
Metabo_species2 <- Metabo_species[,29:ncol(Metabo_species)]

spe.bray <- vegdist(Metabo_species2) #Calculer la distance de Bray entre les molécules et on verra si corrélé aussi avec
...
#Regarder s'il y a un effet de la matrice environnementale sur cette distance, on rajoute les coordonnées (les 3 axes) aux
#variables environnementales. On ajoute des coordonnées (de Bray donc de distance) à mes fétuques.
...
#prendre la PC1 (axe x) pour les sites (1ère colonne de dudi$li donc axe 1 = PC1)
...
sp_div_mean <- tapply(Metabo_stat$div_sp ,Metabo_stat$Plant.Species, mean) ## calculer la moyenne de phytodiversité par espèces
...
Metabo_stat <- Metabo_full_sp1[is.na(Metabo_full_sp1$Altitude),] ## delet ! to exclude rubra field data  # ! to keep rubra
#Sélectionner tous ceux qui ont des "NA" dans la colonne "Altitude"
...
############## phylogeny ####################################################################
#Espèces dans l'ordre de leur optimum de niche avec la moyenne de leur diversité phytochimique.
...
#Signifie probablement : Les points proches sont proches phylogénétiquement. Ici on voit que les
#points proches n'ont pas forcément la même niche (car couleur correspond à l'élévation de la niche)
#ni forcément la même richesse moléculaire (car pas à la même hauteur sur l'axe sp_div_mean).
#C'est donc pour voir si : Si structurellement proches, phylogénétiquement aussi ?
#Ici on voit que c'est surtout l'axe 2 (X2) qui montre un motif malgré une partie bruit mais dans une
#partie il y a un signal donc c'est pas la phylogénie qui structure le + (sinon il y aurait un motif
...
sp_struc <- tapply(Metabo_stat_full_sp$X1, Metabo_stat_full_sp$Plant.Species, mean) ## calculer la moyenne de structure chimique par espèces
...
heatmap(phylo_Matrix, chimie_Matrix)#mantel.test pour faire des corrélations sur des distances
...
setwd("D:/Affaires Nadline/Master thesis/Statistiques/Références") 
...
#Un peu structuré par optiumum de niche (= climat où on a + de chance de trouver l'espèce) mais signal
#moins fort.

##correlation diversité et structure des composés chimiques par espèce et optimum de niche
...
#Ici on a une variable catégorique (cat) alors l'autre (div_sp)
...
deg2 <- lm(Rubra$div_sp~Rubra$Altitude) #Meilleur modèle selon AIC plus bas
deg3 <- lmer(div_sp~Altitude+(1|Transect), data = Rubra)
#AIC

AIC(model_null)
AIC(deg1)
AIC(deg2) #Meilleur modèle
AIC(deg3)
AIC(deg4)
AIC(deg5)
#Pour les species###############
...
setwd("D:/Affaires Nadline/Master thesis/Statistiques/Corrélations Rubra")
...
qqline(Rubra$Altitude) #Considéré comme distribution normale car symétrique 
...
summary(reg)
anova(reg)
#Corrélations entre continuous variables###############################################
...
#(3) PCA des traits fonctionnels (SLA et WC) et du climat en fonction de l'altitude et par catégories
#d'élévation #########################################################################################
...
#(4) PCA des traits fonctionnels (SLA et WC), climat et div_sp en fonction de l'altitude et par catégories
#d'élévation#######################################################################################
...
ggbiplot(Rubra.pca, obs.scale = 1, var.scale = 1, groups = Rubra$Elevation, labels = Rubra$Transect, ellipse = TRUE, circle = FALSE)+
  ...
  theme_classic()
#On voit une corrélation négative entre div_sp et climat!
...
#Vérification des données pour faire les tests stat
...
summary(reg)
anova(reg) #Climat est le même pour chaque plante.species donc pas besoin de le mettre en facteur random
...
#(2) Water Content en fonction espèce
...
#de water content par espèce et pas par individu.

#Leaf_C.N En fonction espèce

reg <- lmer(Leaf_C.N~DEM_Mean+(1|Plant.Species), data=JB_CHN)
summary(reg)
anova(reg)

#(3) div_sp en fonction espèce
...
#Sans catégories mais moyenne des points (points)
Rubra$cat2 <-cut(Rubra$Altitude , breaks=4, right=FALSE, labels=c(1:4))
